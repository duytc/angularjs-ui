(function() {
    ...
    angular.module('tagcade.blocks.queryBuilder')
        .constant('CONDITIONS_NUMERIC', [
            ...
            {key: '<', label: 'LESS THAN', jsPattern: '({VARIABLE} < {VALUE}) '},
            {key: '<=', label: 'LESS THAN OR EQUAL TO', jsPattern: '({VARIABLE} <= {VALUE}) '},
            {key: '>', label: 'GREATER THAN', jsPattern: '({VARIABLE} > {VALUE}) '},
            {key: '>=', label: 'GREATER THAN OR EQUAL TO', jsPattern: '({VARIABLE} >= {VALUE}) '}
        ])
