(function () {
    ...
    angular.module('tagcade.blocks.pubvantageMultiSelect')
        .directive('pubvantageMultiSelect', pubvantageMultiSelect)
        .run(initDropdown)
    ;

    function pubvantageMultiSelect($sce, $timeout, $templateCache) {
        return {
            restrict:
                'AE',
            ...
            link: function ( $scope, element, attrs ) {
                ...
                $scope.backupCurrentModel = angular.copy($scope.inputModel);
                ...
                $scope.clearClicked = function( e ) {
                    $scope.inputLabel.labelFilter = '';
                    $scope.updateFilter();
                    $scope.inputModel = $scope.backupCurrentModel;
                ...
                $scope.searchChanged = function() {
                    ...
                    clearTimeout(seachValue);

                    seachValue = setTimeout(function () {
                        $scope.updateFilter();
                    }, 500);
                }
                ...
                $scope.updateFilter = function()
                {
                    ...
                    $timeout( function() {
                        ...
                        if ( $scope.inputLabel.labelFilter.length > vMinSearchLength ) {
                            ...
                            if($scope.oldFilter !== $scope.inputLabel.labelFilter)
                            {
                                ...
                                $scope.oldFilter = $scope.inputLabel.labelFilter;
                            }
                        }
                    },0);
                };
                ...
                if ( typeof attrs.translation !== 'undefined' ) {
                    ...
                    $scope.lang.doSearch        = $sce.trustAsHtml( $scope.icon.selectAll      + '&nbsp;&nbsp;Search' );
    ...
    function initDropdown($templateCache) {
        var template =
            ...
            // do search
            '<button type="button" class="helperButton"' +
            'ng-click="select( \'search\', $event );"' +
            'ng-bind-html="lang.doSearch">' +
            '</button>' +
            '</div>' +
            // the search box
            ...
            '</div>'+
            '</span>';
        $templateCache.put( 'pubvantage-multi-select.htm' , template );
    }
})();